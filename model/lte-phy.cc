...
LtePhy::GetSrsPeriodicity (uint16_t srcCi) const
{
  ...
  uint16_t SrsCiHigh[16] =      {0, 1, 6, 16, 36, 76, 156, 316, 636, 1276, 2556, 5116, 10236, 20476, 40956, 65535};
  uint8_t i;
  for (i = 15; i > 0; i --)
    {
...
{
  NS_LOG_INFO("Added macpdu to m_packetBurstQueue at size " << m_packetBurstQueue.size () - 1 << " at time " << Simulator::Now().GetSeconds());
