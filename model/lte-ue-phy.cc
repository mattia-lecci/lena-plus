...
LteUePhy::DoNotifyConnectionExpired() 
{
  ...
  NS_LOG_INFO(this << " phy disconnected at time " << Simulator::Now().GetSeconds());
}
...
LteUePhy::CreateDlCqiFeedbackMessage (const SpectrumValue& sinr)
{
  ...
  NS_LOG_INFO(m_rnti << " rnti in CreateDlCqiFeedbackMessage");
  msg->SetDlCqi (dlcqi);
  return msg;
}
...
{
  NS_LOG_INFO (this << " UE frame " << frameNo << " subframe " << subframeNo);
      ...
      if (m_realPrach && LtePrachInfo::IsPrachSff(frameNo, subframeNo)) 
        {
          ...
          if (m_rachMessageReceivedFromMac) // m_rachMessageReceivedFromMac is true after m_rachConfigured becomes true
            { 
              ...
              // in order to be consisten with the MAC update raRnti
