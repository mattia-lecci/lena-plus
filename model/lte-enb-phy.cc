  ...
  for (it = msgList.begin (); it != msgList.end (); it++)
    {
      switch ((*it)->GetMessageType ())
        ...
        case LteControlMessage::RACH_PREAMBLE:
          {
            NS_LOG_DEBUG("Received rach preamble");
      ...
      if (it2 == m_ueAttached.end ())
      ...
      else
        {
          ...
          if ((*dciIt).GetDci ().m_ndi==1)
            {
              NS_LOG_INFO (this << " RNTI " << (*dciIt).GetDci ().m_rnti << " NEW TB expected, added at time " << Simulator::Now().GetSeconds());
            }
          else
            {
              NS_LOG_INFO (this << " RNTI " << (*dciIt).GetDci ().m_rnti << " HARQ RETX expected, added at time " << Simulator::Now().GetSeconds());
            }
        }
    }
