...
{
  NS_LOG_FUNCTION(this << "Imsi " << m_imsi << " RNTI " << m_rnti << " in DoDisconnect at time " << Simulator::Now().GetSeconds());
  switch (m_state)
    {
    case IDLE_START:
      ...
      if (m_srb1 == 0)
        {
          ...
          NS_LOG_INFO("lcid added " << (uint32_t)lcid);
          m_cmacSapProvider->AddLc (lcid, lcConfig, rlc->GetLteMacSapUser ());
...
LteUeRrc::LeaveConnectedMode ()
{
  ...
  m_connectionPending = false;
  SwitchToState (IDLE_CAMPED_NORMALLY);
  NS_LOG_DEBUG ("UE RRC: Now the node with Imsi " << m_imsi << " RNTI " << m_rnti << " is in idle_camped_normally");
}
...
LteUeRrc::ConnectionTimeout ()
{
  NS_LOG_FUNCTION(this << " connection timeout for msg3 for Imsi " << m_imsi);
  m_connectionTimeout.Cancel();
  m_cmacSapProvider->NotifyConnectionExpired ();       // reset the MAC
  m_hasReceivedSib2 = false;         // invalidate the previously received SIB2
  SwitchToState (IDLE_CAMPED_NORMALLY);
